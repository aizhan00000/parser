...
def get_content(html):
    ...
    for item in items:
        ...
        if uah_price:
            uah_price = uah_price.get.text().replace(' . ', ' ')
        else:
            uah_price = 'цены нет'
        cars.append({
            'title': item.find('div', class_='na-card-name').get_text(strip=True),
            'link':  HOST + item.find('span', class_='link').get_text('href'),
            'usd_price': item.find('strong', class_='green').get_text(),
            'uah_price': uah_price,
            'city': item.find('svg', class_='svg_i16_pin').find_next('span').get.text(),
...
def save_file(items, path):
    with open(path, 'w', newline='') as file:
        ...
        for item in items:
            writer.writerow([
                item['title'], item['link'], item['usd_price'],
